...
export type IPhoneModel = "14" | "14-pro" | "15" | "15-pro" | "x" | "plain";
...
/** Outer (frame) size of a model at scale 1 — handy to compute a fitting scale. */
export function getIPhoneOuterSize(
  model: IPhoneModel = "14-pro",
  orientation: IPhoneOrientation = "portrait",
  bezel?: number,
) {
  const spec = DEVICE_SPECS[model];
  const b = bezel ?? spec.bezel;
  const w = orientation === "landscape" ? spec.h : spec.w;
  const h = orientation === "landscape" ? spec.w : spec.h;
...
export const IPhoneMockup: React.FC<IPhoneMockupProps> = ({
  ...
  // The wrapper reserves the *scaled* size so the surrounding layout doesn't
  // keep a gap (`transform: scale` alone doesn't change layout size).
